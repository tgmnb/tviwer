...
api_semaphore = asyncio.Semaphore(value=min(2 * cpu, 8))
...
# 清理最新的接口数据
# zip_list = glob(os.path.join(root_path, 'swap_*m', 'monthly_klines', '*',  f'*newest.zip'))
...
def transfer_daily_to_monthly_and_get_newest(newest_timestamp, daily_list, need_analyse_set, download_folder, trade_type):
    ...
    if trade_type == 'swap':
        # 获取最新的symbol list
        all_trading = asyncio.get_event_loop().run_until_complete(get_all_trading_usdt())
        # 选取所有TRADING的symbol，USDT
        symbol_list_trading = [x['symbol'] for x in all_trading if x['status'] == 'TRADING' and x['symbol'].endswith('USDT')]
        # 复制一份，因为有1m和5m两个interval，后面的for循环会删除2次
        symbol_list_trading += symbol_list_trading
    for local_path, items in groupby(sorted_list, key=itemgetter('local_path')):
        zip_files = []
        day_set = set()
        interval = ''
        symbol = os.path.basename(local_path)
        if symbol in symbol_list_trading:
            symbol_list_trading.remove(symbol)
    ...
    if symbol_list_trading:
        symbol_list_trading = list(set(symbol_list_trading))
        print('\n以下交易对刚上架：', symbol_list_trading)
        for s in symbol_list_trading:
            onboard_timestamp = newest_timestamp - 72 * 60 * 60 * 1000
            for item in all_trading:
                if item['symbol'] == s:
                    onboard_timestamp = item['onboardDate']
                    print(f'{s}上架时间：', datetime.datetime.fromtimestamp(onboard_timestamp / 1000))
                    break
            for interval in ['1m', '5m']:
                monthly_path = get_local_path(download_folder, trade_type, 'klines', 'monthly', s, interval)
                if not os.path.exists(monthly_path):
                    os.makedirs(monthly_path)
                tasks.append(get_klines_from_aio_api(s, interval, onboard_timestamp, newest_timestamp,
                                                 os.path.join(monthly_path, f'{s}-{interval}-newest.zip')))
    print('\n合并结束')
    return tasks
...
def build_hour_offset_pkl(symbol, trading_type, offset, df_big, df_1m):
    ...
    df = df_big.resample(rule=period).agg(agg_dict)
    ...
    if trading_type == 'swap':
        ...
        df['funding_rate_r'] = df['funding_rate_raw']
...
def data_center_symbol_process(symbol, trading_type, zip_path_1m, zip_path_5m):
    ...
    if trading_type == 'swap':
        ...
        funding_df.reset_index(inplace=True)  # 重置index
        funding_df['candle_begin_time'] = pd.to_datetime(funding_df['fundingTime'], unit='ms') - datetime.timedelta(
            hours=1)
        funding_df.set_index('candle_begin_time', inplace=True)
        ...
        # 读取 metrics
        # local_path = get_local_path(root_path, trade_type, 'metrics', 'daily', original_symbol, '5m')
        # # print('metrics ', local_path)
        # zip_list = glob(os.path.join(local_path, f'{original_symbol}-metrics*.zip'))
        # if zip_list:
        #     m_df = pd.concat(
        #         Parallel(1)(
        #             delayed(pd.read_csv)(
        ...
        #     m_df.drop_duplicates(subset=['create_time'], inplace=True, keep='last')
        #     m_df.set_index('create_time', inplace=True)
        #
        #     m_df = m_df.resample(period).agg({
        #         'sum_open_interest': 'last',
        #         'sum_open_interest_value': 'last'
        #     })
        #     df['sum_open_interest'] = m_df['sum_open_interest']
        #     df['sum_open_interest_value'] = m_df['sum_open_interest_value']
...
def spot_symbols_filter(symbols):
    others = []
    stable_symbol = ['BKRW', 'USDC', 'USDP', 'TUSD', 'BUSD', 'FDUSD', 'DAI', 'EUR', 'AEUR', 'GBP']
...
def run():
    ...
    tasks = transfer_daily_to_monthly_and_get_newest(newest_timestamp, daily_list_1m + daily_list_5m, need_analyse_set, download_folder, trade_type)
    if len(tasks) > 0:
        asyncio.get_event_loop().run_until_complete(asyncio.gather(*tasks))
...
def analyse_and_resample_metric_data(download_folder, metrics_symbols):
    ...
    for symbol in sorted(metrics_symbols):
        ...
        k_df = k_df[
            (k_df['candle_begin_time'] >= '2021-12-01 00:00:00') & (k_df['candle_begin_time'] < '2024-09-23 00:00:00')]
...
async def get_all_trading_usdt():
    """
    获取所有交易对
    """
    async with aiohttp.ClientSession() as session:
        ...
        while retry_count < 3:
            try:
                async with session.get(url='https://fapi.binance.com/fapi/v1/exchangeInfo', proxy=proxy, timeout=5) as response:
                    if response.status == 200:
                        t = await response.text()
                        return json.loads(t)['symbols']
            except Exception as e:
                retry_count += 1
                if retry_count >= 3:
                    print('币安接口fapi.binance.com无法连接，请检查网络配置后重新运行', e)
